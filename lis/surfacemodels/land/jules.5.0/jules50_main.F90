            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  do i=1, jules50_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules50_struc(n)%jules50(t)%p_s_sthu(i)+jules50_struc(n)%jules50(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then 
                        smc_liq = jules50_struc(n)%jules50(t)%p_s_sthu(i) / smc_sat_frac
                     else
                        smc_liq = 0.0
                     endif 
                  ...
                  do i=1, jules50_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules50_struc(n)%jules50(t)%p_s_sthu(i)+jules50_struc(n)%jules50(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then
                        smc_ice = jules50_struc(n)%jules50(t)%p_s_sthf(i) / smc_sat_frac
                     else
                        smc_ice = 0.0
                     endif
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SWE,                                       &
                      value =  jules50_struc(n)%jules50(t)%snow_mass_ij, &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     LIS_MOC_EMISSFORC,                                 &
                     value = jules50_struc(n)%jules50(t)%emis_tile(pft),&
                     vlevel=1, unit="-", direction="-",                 &
                     surface_type = LIS_rc%lsm_index)

                  !***  surface radiative temperature 
                  trad_gb_mean = 0.0
                  do p=1, jules50_struc(n)%jules50(t)%nsurft
                    trad_gb_mean = trad_gb_mean + frac(1,p) * jules50_struc(n)%jules50(t)%tstar_tile(p)**4.0
                  enddo
                  ...
                  lw_net    = jules50_struc(n)%jules50(t)%emis_tile(pft) * jules50_struc(n)%jules50(t)%lwdown - lw_upward 
                  call LIS_diagnoseSurfaceOutputVar(n, t,                          &
...
! EMK BEGIN New output variables
                  ...
                  prcp = jules50_struc(n)%jules50(t)%rainf/jules50_struc(n)%forc_count
                  ...
                  cpcp   = jules50_struc(n)%jules50(t)%rainf_c/jules50_struc(n)%forc_count
...
!                 ! JULES GPP and NPP
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_GPP, value = 1000.0 * jules50_struc(n)%jules50(t)%gpp, &
                  ...
                  ! (g m-2 s-1) = 1000.0 * (kg m-2 s-1)
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_NPP, value = 1000.0 * jules50_struc(n)%jules50(t)%npp, &
                                              vlevel=1, unit="g m-2 s-1", direction="IN", surface_type = LIS_rc%lsm_index)
...
! EMK END New output variables
   ...
   ! EMK...See if jules50_struc(n)%jules50(t)%tair_agl_min needs to be reset
   ! for calculating RHMin.  
   write(fnest,'(i3.3)') n
   alarmCheck = LIS_isAlarmRinging(LIS_rc, &
        "JULES.5.0 RHMin alarm "//trim(fnest))
   if (alarmCheck) then
      write(LIS_logunit,*)'[INFO] Resetting tair_agl_min for RHMin calculation'
     do t = 1,LIS_rc%npatch(n,LIS_rc%lsm_index)
        jules50_struc(n)%jules50(t)%tair_agl_min = 999.
     end do
  end if
