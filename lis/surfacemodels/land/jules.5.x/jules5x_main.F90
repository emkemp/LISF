...
!BOP
!
! !ROUTINE: jules5x_main
! \label{jules5x_main}
!
! !ROUTINE: jules5x_main.F90
!
! !REVISION HISTORY:
...
! !DESCRIPTION:
!
!  Calls the run routines for the forcing-only option (jules5x)
!
!  The arguments are:
!  \begin{description}
!  \item[n]
...
!EOP
   ...
   logical         :: get_forcing
   real            :: lw_upward, lw_net, trad, snow_frac, albedo_land, prcp, cpcp, smc_liq, smc_ice, trad_gb_mean
   ...
   alarmCheck = LIS_isAlarmRinging(LIS_rc, "JULES model alarm")
   if (alarmCheck) Then
      do m = 1, LIS_rc%nensem(n)
         ...
         do
            ...
            start_k = k
            end_k = jules5x_struc(n)%jules5x(start_k)%end_k 
            k = end_k + 1 
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                 ...
                 jules5x_struc(n)%jules5x(t)%frac(pft) = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%fgrd
                 tstar_box=tstar_box+jules5x_struc(n)%jules5x(t)%frac(pft)*jules5x_struc(n)%jules5x(t)%tstar_tile(pft)
               endif
            enddo
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  pft = jules5x_struc(n)%jules5x(t)%pft 
                  jules5x_struc(n)%jules5x(t)%frac(pft) = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%fgrd
                  jules5x_struc(n)%jules5x(t)%land_mask = .true.
                  jules5x_struc(n)%jules5x(t)%z1_tq = jules5x_struc(n)%z1_tq
                  jules5x_struc(n)%jules5x(t)%z1_uv = jules5x_struc(n)%z1_uv

                  jules5x_struc(n)%jules5x(t)%tstar=tstar_box
               ...
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  do i=1, jules5x_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,         &
                        LIS_MOC_SOILTEMP,                            &
                        value=jules5x_struc(n)%jules5x(t)%t_soil(i), &
                  ...
                  do i=1, jules5x_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,       &
                        LIS_MOC_SOILMOIST,                         &
                        value=jules5x_struc(n)%jules5x(t)%smcl_soilt(i), &
...
!                  do i=1, jules5x_struc(n)%sm_levels
!                     smc_liq = jules5x_struc(n)%jules5x(t)%p_s_sthu(i) * jules5x_struc(n)%jules5x(t)%p_s_smvcst(i) 
!                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
!                        LIS_MOC_SMLIQFRAC,                            &
!                        value=smc_liq, &
!                        vlevel=i, unit="m^3 m-3", direction="-",             &
...
!                  do i=1, jules5x_struc(n)%sm_levels
!                     smc_liq = jules5x_struc(n)%jules5x(t)%p_s_sthu(i)/(jules5x_struc(n)%jules5x(t)%p_s_sthu(i)+jules5x_struc(n)%jules5x(t)%p_s_sthf(i))
!                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
!                        LIS_MOC_SMLIQFRAC,                            &
!                        value=smc_liq, &
...
!                  do i=1, jules5x_struc(n)%sm_levels
!                     smc_ice = jules5x_struc(n)%jules5x(t)%p_s_sthf(i) * jules5x_struc(n)%jules5x(t)%p_s_smvcst(i) 
!                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
!                        LIS_MOC_SMFROZFRAC,                            &
!                        value=smc_ice, &
!                        vlevel=i, unit="m^3 m-3", direction="-",             &
...
!                  do i=1, jules5x_struc(n)%sm_levels
!                     smc_ice = jules5x_struc(n)%jules5x(t)%p_s_sthf(i)/(jules5x_struc(n)%jules5x(t)%p_s_sthu(i)+jules5x_struc(n)%jules5x(t)%p_s_sthf(i))
!                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
!                        LIS_MOC_SMFROZFRAC,                            &
!                        value=smc_ice, &
!                        vlevel=i, unit="-", direction="-",             &
...
!                  enddo
                  ...
                  do i=1, jules5x_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHU,                            &
                        value=jules5x_struc(n)%jules5x(t)%p_s_sthu(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  do i=1, jules5x_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHF,                            &
                        value=jules5x_struc(n)%jules5x(t)%p_s_sthf(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     ...
                     value=jules5x_struc(n)%jules5x(t)%p_s_sthu_min(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules5x_struc(n)%jules5x(t)%p_s_smvccl(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules5x_struc(n)%jules5x(t)%p_s_smvcst(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules5x_struc(n)%jules5x(t)%p_s_smvcwt(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_EVAP,                              &
                     value=jules5x_struc(n)%jules5x(t)%fqw_1,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,     &
                     LIS_MOC_QH,                              &
                     value=jules5x_struc(n)%jules5x(t)%ftl_1, &
                     vlevel=1,unit="W m-2",direction="UP",    &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_ECANOP,                             &
                     value=jules5x_struc(n)%jules5x(t)%ecan,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_JULES_ESOIL,                             &
                     value=jules5x_struc(n)%jules5x(t)%esoil,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_JULES_ESOIL,                             &
                     value=jules5x_struc(n)%jules5x(t)%esoil*dt,     &
                     vlevel=1,unit="kg m-2", direction="UP", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,       &
                     LIS_MOC_SUBSNOW,                          &
                     value=jules5x_struc(n)%jules5x(t)%ei,     &
                  ...
                  do i=1, jules5x_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n,t,             &
                        LIS_MOC_SOILET,                                 &
                        vlevel=i,                                       &
                        value=jules5x_struc(n)%jules5x(t)%ext(i),       &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,               &
                     LIS_MOC_SNOWDEPTH,                                &
                     vlevel=1,                                         &
                     value=jules5x_struc(n)%jules5x(t)%snowdepth(pft), &
                     unit="m", direction="-",&
                     surface_type=LIS_rc%lsm_index)

                  !(unit=mm) ***  snow water equivalent, snow_mass_ij (kg/m2) equivalent to LIS SWE
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SWE,                                       &
                      value =  jules5x_struc(n)%jules5x(t)%snow_mass_ij, &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,              &
                      LIS_MOC_WATERTABLED,                             &
                      value = JULES5x_struc(n)%jules5x(t)%zw,          &
                      vlevel=1, unit="m", direction="-",               &
                      surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                &
                     LIS_MOC_AVGSURFT,                                  &
                     value=jules5x_struc(n)%jules5x(t)%tstar_tile(pft), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     LIS_MOC_EMISSFORC,                                 &
                     value = jules5x_struc(n)%jules5x(t)%emis_tile(pft),&
                     vlevel=1, unit="-", direction="-",                 &
                     surface_type = LIS_rc%lsm_index)

                  !***  surface radiative temperature 
                  trad_gb_mean = 0.0
                  do p=1, jules5x_struc(n)%jules5x(t)%nsurft
                    trad_gb_mean = trad_gb_mean + frac(1,p) * jules5x_struc(n)%jules5x(t)%tstar_tile(p)**4.0
                  enddo
                  ...
                  lw_upward = sbcon * jules5x_struc(n)%jules5x(t)%emis_tile(pft) * jules5x_struc(n)%jules5x(t)%tstar_tile(pft)**4 
                  lw_net    = jules5x_struc(n)%jules5x(t)%emis_tile(pft) * jules5x_struc(n)%jules5x(t)%lwdown - lw_upward 
                  call LIS_diagnoseSurfaceOutputVar(n, t,                          &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,                 &
                     LIS_MOC_ROUGHNESS,                                  &
                     value=jules5x_struc(n)%jules5x(t)%p_s_z0_tile(pft), &
                     vlevel=1,unit="m",direction="-",                    &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                       &
                     LIS_MOC_THERMAL_ROUGHNESS,                                &
                     value=jules5x_struc(n)%jules5x(t)%p_s_z0_tile(pft) * z0h_z0m(pft), &
                     vlevel=1,unit="m",direction="-",                          &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSB,                                             &
                     value = jules5x_struc(n)%jules5x(t)%sub_surf_roff,       &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  ! EMK
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QS,                              &
                     value=jules5x_struc(n)%jules5x(t)%surf_roff*dt,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QSB,                              &
                     value=jules5x_struc(n)%jules5x(t)%sub_surf_roff*dt,   &
                     vlevel=1,unit="kg m-2",direction="OUT", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QS,                                              &
                     value = jules5x_struc(n)%jules5x(t)%surf_roff,           &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSM,                                             &
                     value = jules5x_struc(n)%jules5x(t)%snow_melt_gb,        &
                     vlevel=1, unit="kg m-2 s-1", direction="S2L", surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSM,                                             &
                     value = jules5x_struc(n)%jules5x(t)%snow_melt_gb * dt,   &
                  ...
                  !call LIS_diagnoseSurfaceOutputVar(n,t,                      &
                  !   LIS_MOC_INFILTRATION,                                    &
                  !   value=jules5x_struc(n)%jules5x(t)%tot_infil,             &
                  !   vlevel=1,unit="kg m-2 s-1",direction="-",                &
                  !   surface_type=LIS_rc%lsm_index)
                  
                  !call LIS_diagnoseSurfaceOutputVar(n,t,                      &
                  !   LIS_MOC_INFILTRATION,                                    & 
                  !   value=jules5x_struc(n)%jules5x(t)%tot_infil * dt,        &
                  !   vlevel=1,unit="kg m-2",direction="-",                    &
                  !   surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_CANOPINT,                                        &
                     value = jules5x_struc(n)%jules5x(t)%canopy(pft),      &
                     vlevel=1, unit="kg m-2", direction="-", &
                     surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_SWNET,                                           &
                     value= jules5x_struc(n)%jules5x(t)%sw_tile(pft),         &  
                     vlevel=1,  unit="W m-2", direction="DN", surface_type=LIS_rc%lsm_index)

                  if(pft <= jules5x_struc(n)%npft) then  
                    ! vegetation surface types 
                    call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                       LIS_MOC_LAI,                                             &
                       value= jules5x_struc(n)%jules5x(t)%lai(pft),             &  
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FSAT,                                             &
                     value= jules5x_struc(n)%jules5x(t)%fsat,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FWETL,                                             &
                     value= jules5x_struc(n)%jules5x(t)%fwetl,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
...
! EMK BEGIN New output variables
                  ...
                  sfcprs = jules5x_struc(n)%jules5x(t)%psurf
                  sfctmp = jules5x_struc(n)%jules5x(t)%tair
                  q2 = jules5x_struc(n)%jules5x(t)%qair
                  es = 611.0*exp(2.501E6/461.0*(1./273.15 - 1./sfctmp))
                  q2sat = 0.622*es/(sfcprs-(1.-0.622)*es)
                  if (sfctmp .lt. &
                       jules5x_struc(n)%jules5x(t)%tair_agl_min) then
                     jules5x_struc(n)%jules5x(t)%tair_agl_min = sfctmp
                     jules5x_struc(n)%jules5x(t)%rhmin = q2 / q2sat
                  end if

                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=jules5x_struc(n)%jules5x(t)%rhmin,   &
                     vlevel=1,unit="-",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=(jules5x_struc(n)%jules5x(t)%rhmin*100),   &
                     vlevel=1,unit="%",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  if(nsmax .eq.0) then
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWICE,                         &
                       value=jules5x_struc(n)%jules5x(t)%sice(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow liquid water 
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWLIQ,                         &
                       value=jules5x_struc(n)%jules5x(t)%sliq(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow temperature
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWT,                         &
                       value=jules5x_struc(n)%jules5x(t)%tsnow(1,1), &
                       vlevel=1, unit="K", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! bulk snow density 
                    call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENSITY, &
                        value =  jules5x_struc(n)%jules5x(t)%rho_snow_grnd(pft),&
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRAIN, &
                        value =  jules5x_struc(n)%jules5x(t)%rgrain(pft),&
                    ...
                    do i=1, jules5x_struc(n)%nsmax
                      ! snow ice 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWICE,                         &
                          value=jules5x_struc(n)%jules5x(t)%sice(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! snow liquid water 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWLIQ,                         &
                          value=jules5x_struc(n)%jules5x(t)%sliq(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! snow temperature K 
                       call LIS_diagnoseSurfaceOutputVar(n, t,            &
                          LIS_MOC_SNOWTPROF,                              &
                          value=jules5x_struc(n)%jules5x(t)%tsnow(pft,i), &
                          vlevel=i, unit="K", direction="-",              &
                          surface_type=LIS_rc%lsm_index)
                      ! snow density 
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENSITY, &
                        value = jules5x_struc(n)%jules5x(t)%rho_snow(pft,i),&
                        vlevel=i, unit="kg m-3", direction="-", surface_type = LIS_rc%lsm_index)
                      
                      ! snow grain size 
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRAIN, &
                        value =  jules5x_struc(n)%jules5x(t)%rgrainl(pft,i),&
                      ...
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWTHICK, &
                        value = jules5x_struc(n)%jules5x(t)%ds(pft,i),& 
                  ...
                  snow_frac = 0.0
                  if(l_aggregate) then
                    if(jules5x_struc(n)%jules5x(t)%snow_tile(1)+jules5x_struc(n)%jules5x(t)%snow_grnd(1)>1.0) then
                      snow_frac = 1.0
                    endif
                  else
                    if(jules5x_struc(n)%jules5x(t)%snow_tile(pft)+jules5x_struc(n)%jules5x(t)%snow_grnd(pft)>1.0) then
                      snow_frac = jules5x_struc(n)%jules5x(t)%frac(pft)
                    endif
                  endif
                  ...
                  cpcp   = jules5x_struc(n)%jules5x(t)%rainf_c/jules5x_struc(n)%forc_count
                  ...
                  albedo_land = wght_alb(1) * jules5x_struc(n)%jules5x(t)%land_albedo(1) &
                              + wght_alb(2) * jules5x_struc(n)%jules5x(t)%land_albedo(2) &
                              + wght_alb(3) * jules5x_struc(n)%jules5x(t)%land_albedo(3) &
                              + wght_alb(4) * jules5x_struc(n)%jules5x(t)%land_albedo(4) 
...
!                 ! JULES GPP and NPP
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_GPP, value = 1000.0 * jules5x_struc(n)%jules5x(t)%gpp, &
                  ...
                  ! (g m-2 s-1) = 1000.0 * (kg m-2 s-1)
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_NPP, value = 1000.0 * jules5x_struc(n)%jules5x(t)%npp, &
                                              vlevel=1, unit="g m-2 s-1", direction="IN", surface_type = LIS_rc%lsm_index)
!                ! End JULES GPP and NPP 

!                   !EMK Below code is borrowed from Noah 3.6
                  do i=1, jules5x_struc(n)%sm_levels
                     smc = jules5x_struc(n)%jules5x(t)%smcl_soilt(i)/ &
                          (1000.0*dzsoil(i))
                     smcwilt = jules5x_struc(n)%jules5x(t)%sm_wilt
                     smcmax = jules5x_struc(n)%jules5x(t)%sm_sat
...
! EMK END New output variables
                  ...
                  jules5x_struc(n)%jules5x(t)%rainf_c = 0.0
