...
|"`SNIP analysis`"   |USAF SNow and Ice Product
...
|"`OPL WSF brightness temperature resampling`" |Operational WSF-M (Weather Satellite Follow-on) brightness temperature resampling to the model grid
|===
...
`Routing model:` specifies the river routing model used in a LIS run.  HYMAP, HYMAP2 and HYMAP3 routing scheme parameters are supported in LDT.
...
|"`WSF soil moisture`" | WSF-M (Weather Satellite Follow-on) soil moisture retrievals from OPL pipeline (units in "`m3/m3`")
...
`LIS SWS output processing:` specifies whether to take surface water storage (SWS) computed by LIS with the HYMAP2 and HYMAP3 routing models into account when processing GRACE data. For more information about generating SWS inputs, please refer to Sections "`HYMAP2 Routing`", "`HYMAP3 Routing`" and "`Model Output Specifications`" in the _LIS Users' Guide_. When this setting is enabled, LDT looks for the LIS-generated SWS data in `./OUTPUT/ROUTING`. For more information about this feature, please refer to https://doi.org/10.1002/2017GL074684 and https://doi.org/10.1029/2019WR026259. Default value is 0. Acceptable values are:
...
`WSF soil moisture data directory:` specifies the location of the directory containing the hourly WSF-M soil moisture retrieval files (`ARFS_SM_WSFM_*.nc`) generated by the WSF OPL pipeline.

.Example _ldt.config_ entry
....
WSF soil moisture data directory:   ./wsf_sm_data
....

[[ssec-hymapopts]]
=== HYMAP routing options

This section describes the parameters needed for the HYMAP, HYMAP2 and HYMAP3 routing models.  The config entries only have "`HYMAP`" within them, as they apply to all HYMAP versions.
...
`HYMAP drainage area map:` specifies the name of the drainage area data file. This parameter is not used in HYMAP. But it is a "good-to-have" for post-processing purposes.

`HYMAP basin map:` specifies the name of the basin map data file. This parameter is not used in HYMAP. But it is a "good-to-have" for post-processing purposes.
...
`HYMAP baseflow dwi ratio map:` specifies the name of the baseflow dwi ratio data file.  This is an optional parameter that is required only when "`HYMAP2 routing model dwi flag`" or "`HYMAP3 routing model dwi flag`" are set to 1 (turned on) in LIS in the _lis.config_ file.  If this flag is set to 0 in the _lis.config_ file, the baseflow dwi ratio map is not required.

`HYMAP runoff dwi ratio map:` specifies the name of the runoff dwi ratio data file.  This is an optional parameter that is required only when "`HYMAP2 routing model dwi flag`" or "`HYMAP3 routing model dwi flag`" are set to 1 (turned on) in LIS in the _lis.config_ file.  If this flag is set to 0 in the _lis.config_ file, the runoff dwi ratio map is not required.

`HYMAP river flow type map:` specifies the name of the river flow type data file.  This is an optional parameter that is required only when "`HYMAP2 routing method`" or "`HYMAP3 routing method`" are set to "`hybrid`" in LIS in the _lis.config_ file.  If the routing method is set to a different option, the river flow type map is not required.

`HYMAP urban drainage outlet map:` specifies the name of the baseflow time delay data file. This is an optional parameter that is required only when "`HYMAP2 routing method`" or "`HYMAP3 routing method`" are set to "`urban`". If the routing method is set to a different option, the urban drainage outlet map is not required.

`HYMAP levee height map:` specifies the name of the baseflow time delay data file. This is an optional parameter that is required only when "`HYMAP2 levee`" or "`HYMAP3 levee:`" are set to "`1`". If the levee option is set to "`0`" or is not specified, the levee map is not required.

`Land grid area map:` specifies the name of the baseflow time delay data file. This parameter is optional and not used in HYMAP routing models. But it is a "good-to-have" for post-processing purposes.

`HYMAP node lon:` specifies the name of the grid node longitude data file. This parameter is optional and not used in HYMAP routing models. But it is a "good-to-have" for post-processing purposes.
...
HYMAP urban drainage outlet map:         ../HYMAP_parms/lis_urban_0125.bin
HYMAP levee height map:                  ../HYMAP_parms/lis_levee_0125.bin
...
`USAFSI source of ocean data:` specifies the source of the USAFSI ocean data.  Acceptable values are:
...
|ESPC-D | Earth System Prediction Capability - Deterministic
|===
...
`USAFSI ESPC-D SST data directory:` directory with ESPC-D sea surface temperature netCDF files

`USAFSI ESPC-D CICE data directory:` directory with ESPC-D sea ice netCDF files
...
[[ssec-snip]]
=== USAF SNow and Ice Product

`SNIP valid date (YYYYMMDDHH):` valid date and hour (UTC) of analysis
`SNIP netcdf data directory for SNIP:` directory for SNIP netCDF files
`SNIP netcdf filename prefix for SNIP:` prefix used in constructing SNIP netcdf filenames
`SNIP netcdf data directory for USAFSI:` directory for USAFSI netCDF files
`SNIP netcdf filename prefix for USAFSI:` prefix used in constructing USAFSI netcdf filenames
`SNIP data directory for modified SNODEP:` directory with legacy SNODEP "`modified`" data
`SNIP data directory for unmodified SNODEP:` directory with legacy SNODEP "`unmodified`" data
`SNIP static data directory:` directory with legacy SNODEP static data

.Example _ldt.config_ entry
....
SNIP valid date (YYYYMMDDHH):                    2025012006
SNIP netcdf data directory for SNIP:       ./input/snip
SNIP netcdf filename prefix for SNIP:      SNIP
SNIP netcdf data directory for USAFSI:     ./input/usafsi
SNIP netcdf filename prefix for USAFSI:    USAFSI
SNIP data directory for modified SNODEP:   ./input/snodep_modified/
SNIP data directory for unmodified SNODEP: ./input/snodep_unmodified/
SNIP static data directory:                ./input/static/
....

`SNIP AMSR2 snowdepth data directory:` directory with AMSR2 retrievals
`SNIP AMSR2 shallow snow depth threshold (m):` threshold to reject AMSR2 retrieval if smaller than first guess

.Example _ldt.config_ entry
....
SNIP AMSR2 snowdepth data directory:        ./input/AMSR2_retrievals/
SNIP AMSR2 shallow snow depth threshold (m): 0.05
....

`SNIP VIIRS data directory:` directory with VIIRS TIFF files
`SNIP max age of VIIRS pixels to use:` used to ignore old VIIRS data
`SNIP min VIIRS fraction to mark point as bare ground:` fraction of no-snow VIIRS pixels in LDT grid box required to mark as "`no-snow`"
`SNIP use VIIRS snow mask:` self-explanatory
`SNIP default snow depth (m) when actual depth unknown:` bogus value for when snow detected but depth unknown

.Example _ldt.config_ entry
....
SNIP VIIRS data directory:                      ./input/viirs/
SNIP max age of VIIRS pixels to use: 3
SNIP min VIIRS fraction to mark point as bare ground:  0.6
SNIP min VIIRS fraction to mark point as snow: 0.4
SNIP use VIIRS snow mask: .true.
SNIP default snow depth (m) when actual depth unknown: 0.1
....

`SNIP GALWEM root directory:` root directory for GALWEM data
`SNIP GALWEM subdirectory:` subdirectory for GALWEM data
`SNIP GALWEM use timestamp directories:` option to use YYYYMMDD in constructing full GALWEM directory; 1 is yes, 0 is no
`SNIP GALWEM nominal resolution (km):` 17 or 10 (17 currently used in operations)
`SNIP LIS GRIB2 data directory:` directory with LIS GRIB2 2-m temperature analyses
`SNIP LIS GRIB2 security class:` used for constructing LIS GRIB2 filename
`SNIP LIS GRIB2 data category:` used for constructing LIS GRIB2 filename
`SNIP LIS GRIB2 data resolution:` used for constructing LIS GRIB2 filename
`SNIP LIS GRIB2 area of data:` used for constructing LIS GRIB2 filename
`SNIP degribbed LIS 0.25 deg sfc temperature data directory:` directory with legacy 0.25 deg LIS ungribbed surface temperatures

.Example _ldt.config_ entry
....
SNIP GALWEM root directory: ./input/GALWEM
SNIP GALWEM subdirectory: GALWEM
SNIP GALWEM use timestamp directories: 1
SNIP GALWEM nominal resolution (km): 17
SNIP LIS GRIB2 data directory:  ./input/LIS_T2/
SNIP LIS GRIB2 security class:  U
SNIP LIS GRIB2 data category:   C
SNIP LIS GRIB2 data resolution: C0P09DEG
SNIP LIS GRIB2 area of data:    GLOBAL
SNIP degribbed LIS 0.25 deg sfc temperature data directory: ./input/sfctmps/
....


`SNIP surface obs data directory:` directory with surface snow reports
`SNIP surface obs data format:` specifies format of surface observation files
[cols="<,<",]
|===
|Value |Description
|1 |Legacy
|2 |WIGOS
|===
`SNIP maximum number of surface observations allowed:` self-explanatory
`SNIP minimum snow depth (m) for which to print a diagnostic:` threshold for printing surface ob report
`SNIP latitudes (deg * 100) for summer climo check:` three latitude bands used for sanity checking snow reports
`SNIP elevations (m) for summer climo check:` four elevations used for sanity checking snow reports
`SNIP temperature (deg K * 10) above which no snow is allowed:` used to adjust snow analysis
`SNIP max reported temperature (deg K * 10) allowed around poles:` used to reject snow ground reports that are too warm
`SNIP observation error variance (m^2):` error variance assigned to surface snow reports
`SNIP background error variance (m^2):` error variance assigned to first-guess field
`SNIP background error horizontal correlation length (m):` correlation length for spreading snow depth corrections horizontally
`SNIP background error vertical correlation length (m):` correlation length for spreading snow depth corrections vertically
`SNIP elevQC difference threshold (m):` threshold for sanity checking snow report elevation against LDT
`SNIP skewed backQC snow depth threshold (m):` threshold for rejecting abnormally small snow depth reports

.Example _ldt.config_ entry
....
SNIP surface obs data directory:                ./input/sfcobs/
SNIP surface obs data format:                   1 # Legacy format
SNIP maximum number of surface observations allowed: 200000
SNIP minimum snow depth (m) for which to print a diagnostic: 0.025
# There should be three latitudes listed below, defining three bands
# for sanity checking snow reports in different seasons.
# Strongly recommend 4000 3000 2000
SNIP latitudes (deg * 100) for summer climo check: 4000 3000 2000
...
SNIP elevations (m) for summer climo check: 1000 1500 1000 1000
SNIP temperature (deg K * 10) above which no snow is allowed: 2820
SNIP max reported temperature (deg K * 10) allowed around poles: 3030
SNIP observation error variance (m^2):  0.0006
SNIP background error variance (m^2):   0.0010
SNIP background error horizontal correlation length (m):  55555.
SNIP background error vertical correlation length (m):      800.
SNIP elevQC difference threshold (m): 400.
SNIP skewed backQC snow depth threshold (m):  0.4
....

`SNIP Snow Climatology:` specifies snow climatology data. Acceptable values are:
[cols="<,<",]
|===
|Value |Description
...
|2 |updated 10-km climo
|===
`SNIP decimal fraction adjustment of snow depth towards climo:` controls drift to climo in data void region
`SNIP bogus climatology snow depth value (m):` bogus value for climatology if not available

.Example _ldt.config_ entry
....
SNIP Snow Climatology:                1  # 1: legacy 2: updated 10-km climo
SNIP decimal fraction adjustment of snow depth towards climo: 0.1
SNIP bogus climatology snow depth value (m): 0.2
....

`SNIP source of ocean data:` specifies the source of the USAFSI ocean data.  Acceptable values are:
[cols="<,<",]
|===
|Value |Description
|ESPC-D | Earth System Prediction Capability - Deterministic
|GOFS   | Global Ocean Forecast System
|===
`SNIP ESPC-D SST data directory:` directory with ESPC-D sea surface temperature netCDF files
`SNIP ESPC-D CICE data directory:` directory with ESPC-D sea ice netCDF files
`SNIP GOFS SST data directory:` directory with GOFS sea surface temperature netCDF files
`SNIP GOFS CICE data directory:` directory with GOFS sea ice netCDF files
`SNIP FNMOC SST GRIB1 data directory:` directory with legacy 0.25 deg FNMOC SST GRIB1 files (used only as emergency backup to GOFS or ESPC-D netCDF data)
`SNIP minimum ice concentration (%) needed to set ice flag:` used when constructing binary sea ice field
`SNIP high latitude thresholds (deg) for sea ice::` 24 latitudes, each row defining a hemisphere (NH, then SH), each column defining a month (J F M A M J J A S O N D). Used to define "`high latitudes`" in sea ice analysis.
`SNIP low latitude thresholds (deg) for sea ice:` 24 latitudes, each row defining a hemisphere (NH, then SH), each column defining a month (J F M A M J J A S O N D). Used to define "`low latitudes`" in sea ice analysis.

.Example _ldt.config_ entry
....
SNIP source of ocean data: "ESPC-D"
SNIP ESPC-D SST data directory:  ./input/ESPC-D/
SNIP ESPC-D CICE data directory: ./input/ESPC-D/
SNIP GOFS SST data directory:   ./input/GOFS/
SNIP GOFS CICE data directory:  ./input/GOFS/
# Backup for when no ESPC-D or GOFS data found
SNIP FNMOC SST GRIB1 data directory: ./input/FNMOC_SST/
SNIP minimum ice concentration (%) needed to set ice flag: 15
# There should 24 latitudes defined below, each row definining a
# hemisphere (NH first row, SH second row), and each column defining a
# month (J F M A M J J A S O N D).  These are used to define high
# latitudes in the sea ice analysis.
SNIP high latitude thresholds (deg) for sea ice::
       81.0 81.0 81.0 81.0 81.0 81.0 81.0 82.0 82.0 82.0 81.0 81.0
       90.0 90.0 78.0 78.0 68.0 68.0 67.0 67.0 66.0 66.0 68.0 70.0
::
# There should 24 latitudes defined below, each row definining a
# hemisphere (NH first row, SH second row), and each column defining a
# month (J F M A M J J A S O N D).  These are used to define low
# latitudes in the sea ice analysis.
SNIP low latitude thresholds (deg) for sea ice::
       45.0 44.0 44.0 45.0 51.0 52.0 55.0 65.0 65.0 62.0 53.0 50.0
       60.0 62.0 62.0 60.0 58.0 56.0 55.0 55.0 53.0 54.0 55.0 57.0
::
....
...
`SMAP_E_OPL snow source:` specifies source of snow outputs
...
=== WSF OPL brightness temperature resampling

This section describes the configuration options for the
"`OPL WSF brightness temperature resampling`" running mode, which resamples
raw WSF-M (Weather Satellite Follow-on) Level-1 SDR brightness temperature files to the
10-km global model grid. One LDT instance is invoked per hour; the
`wsf_opl.py` pipeline script populates the time-varying fields automatically
from a template configuration file.

`WSF valid date (YYYYMMDDHH):` specifies the valid date and hour (UTC) of the
WSF-M SDR granule to process.

.Example _ldt.config_ entry
....
WSF valid date (YYYYMMDDHH):   2025060100
....

`WSF input directory:` specifies the directory containing the raw WSF-M SDR
NetCDF files for the date to be processed. The pipeline substitutes this
per-hour from the source archive.

.Example _ldt.config_ entry
....
WSF input directory:   /path/to/WSF/raw/202506
....

`WSF output directory:` specifies the directory where the resampled hourly
output file (`WSF_SDR_resampled_<YYYYMMDD>_t<HH>.nc`) will be written.

.Example _ldt.config_ entry
....
WSF output directory:   /path/to/WSF_TB_resampled_hourly/202506
....

`WSF filelist suffix number:` specifies a unique integer suffix appended to
the internal WSF filelist to prevent conflicts when multiple LDT jobs run
concurrently in the same working directory. Increment this value for each
parallel job.

.Example _ldt.config_ entry
....
WSF filelist suffix number:   1
....

.Full example _ldt.config_ block for WSF OPL resampling
....
LDT running mode:              "OPL WSF brightness temperature resampling"
