...
!BOP
! !ROUTINE: jules5x_updatesnodep
! \label{jules5x_updatesnodep}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules5x_updatesnodep(n, LSM_State, LSM_Incr_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use jules5x_lsmMod
...
! the good ensemble members. If all else fails, do not update.
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(jules5x_struc(n)%jules5x(t)%snowdepth(pft).gt.0) then
       sndens = jules5x_struc(n)%jules5x(t)%snow_mass_ij / &
           jules5x_struc(n)%jules5x(t)%snowdepth(pft)
     else
       sndens = 0.0
     endif
  ...
  enddo
end subroutine jules5x_updatesnodep
