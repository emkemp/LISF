...
!values
          ios = nf90_get_var(nid,aice_arc_id, aice_arc,&
               start=(/1,1,1,1/), &
               count=(/LVT_rc%HYCOM_aice_arc_nc,LVT_rc%HYCOM_aice_arc_nr,1,1/))
...
!values
          ios = nf90_get_var(nid,aice_ant_id, aice_ant,&
               start=(/1,1,1,1/), &
               count=(/LVT_rc%HYCOM_aice_ant_nc,LVT_rc%HYCOM_aice_ant_nr,1,1/))
...
!EOP
    ...
    call LVT_verify(iret,'grib_set:localTablesVersion failed in LVT_DataStreamsMod')
    ! Octet 12
    ...
    call LVT_verify(iret,&
         'grib_set:productionStatusOfProcessedData failed in LVT_DataStreamsMod')
    ! Octet 21
...
!    call LVT_verify(iret,'grib_set:stepType failed in LVT_DataStreamsMod')
    ...
    if (pdTemplate == 0 .or. pdTemplate == 2 .or. pdTemplate == 12) then
       ...
       call LVT_verify(iret,&
            'grib_set:backgroundGeneratingProcessIdentifier failed in LVT_DataStreamsMod')
       ! Octet 14
       ...
       if (typeOfGeneratingProcess_local == 0) then ! Analysis
          ...
          call LVT_verify(iret,'ESMF_TimeSet:time1 failed in LVT_DataStreamsMod')
          call ESMF_TimeSet(time2, yy=LVT_rc%yr, mm=LVT_rc%mo, dd=LVT_rc%da, &
               h=LVT_rc%hr, m=LVT_rc%mn, s=LVT_rc%ss, rc=iret)
          call LVT_verify(iret,'ESMF_TimeSet:time2 failed in LVT_DataStreamsMod')
          if (pdTemplate == 12) then
             call ESMF_TimeIntervalSet(timeinterval12,&
                  s=LVT_rc%statswriteint,rc=iret)
             call LVT_verify(iret,'ESMF_TimeIntervalSet:timeinterval12 failed in LVT_DataStreamsMod')
       ...
       if ( gribSfc .eq. 106 ) then   ! soil layers
          ...
          call LVT_verify(iret,&
               'grib_set:typeOfSecondFixedSurface failed in LVT_DataStreamsMod')
          ...
          call grib_set(igrib,'scaleFactorOfSecondFixedSurface',depscale(1), &
               iret)
          call LVT_verify(iret,'grib_set:scaledFactorOfSecondFixedSurface failed in LVT_DataStreamsMod')
          call grib_set(igrib,'scaledValueOfSecondFixedSurface',botlev(1), &
               iret)
    ...
    if (pdTemplate == 2 .or. pdTemplate == 12) then
       ...
       call LVT_verify(iret,'grib_set:numberOfForecastsInEnsemble failed in LVT_DataStreamsMod')
