...
!BOP
! !ROUTINE: jules5x_getsnodeppred
! \label{jules5x_getsnodeppred}
!
! !REVISION HISTORY:
! 27Feb2005: Sujay Kumar; Initial Specification
! 25Jun2006: Sujay Kumar: Updated for the ESMF design
! 02 Mar 2010: Sujay Kumar; Modified for Noah 3.1
! 01 May 2014: Yuqiong Liu; modifed to include mesh8, mesh16, and 0p25 SNODEP 
!              data
...
!
! !INTERFACE:
subroutine jules5x_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = jules5x_struc(n)%jules5x(t)%snowdepth(pft)*39.37 !convert from meter to inch
     elseif (SNODEP_obs_obj(n)%mesh .eq. 16 .or. SNODEP_obs_obj(n)%mesh .eq. 25) then
        snwd(t) = jules5x_struc(n)%jules5x(t)%snowdepth(pft)
     endif
  enddo
...
end subroutine jules5x_getsnodeppred
