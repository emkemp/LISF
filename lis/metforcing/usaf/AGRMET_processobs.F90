...
!     20 Aug 21  Added extended station IDs and country IDs..............
!                .....................................Eric Kemp/NASA/SSAI
! !INTERFACE:
...
!   \item[firsts6]         Index to first instance of each network in obs\_6
!   \item[firsts12]        Index to first instance of each network in obs\_12
...
!   \item[lasts6]          Index to last instance of each network in obs\_6
!   \item[lasts12]         Index to last instance of each network in obs\_12
...
!   \item[prior\_net]       The JMOBS of previous ob in obs\_6 or obs\_12 list
...
!     ------------------------------------------------------------------
  ...
  ! EMK 20 Aug 2021
  obs_cur(1)%duration = obs(1)%duration
  obs_cur(1)%country_id = obs(1)%country_id
  count = 1
...
!     ------------------------------------------------------------------
        ...
        if (obs(i)%amtmsc > obs(i-1)%amtmsc) then
           ...
           obs_cur(count)%duration = obs(i)%duration ! EMK 20 Aug 2021
        end if
        ...
        obs_cur(count)%amtmsc = obs(i)%amtmsc
        ! EMK 20 Aug 2021
        obs_cur(count)%duration = obs(i)%duration
        obs_cur(count)%country_id = obs(i)%country_id
     end if DUP_CHECK
...
!-----------------------------------------------------------------------
  ...
  if (sixyes) then
     ...
     do i = 1, count6
        ...
        ! EMK...Support new and old presav files
        if (agrmet_struc(n)%read_expanded_precip_station_ids .eq. 1) then
           read(8, 6001, iostat=istat, err=100, end=100) &
           ...
           read(8, 6000, iostat=istat, err=100, end=100) &
                ...
                obs_6(i)%amt24, obs_6(i)%amt12, obs_6(i)%amt6,&
                obs_6(i)%amtmsc
           obs_6(i)%duration = 99 ! Not preserved in old presav files
           obs_6(i)%country_id = "??"
        end if

        if (obs_6(i)%net .eq. "WMO") then
           ! EMK...Support expanded station IDs
           obs_6(i)%wmonum = &
                set_bsn_wmo(agrmet_struc(n)%read_expanded_precip_station_ids, &
                obs_6(i)%platform, obs_6(i)%country_id)
        else if (obs_6(i)%net .eq. "CDMS") then
           read(obs_6(i)%platform, FMT='(I32)') obs_6(i)%wmonum
        else
           obs_6(i)%wmonum = 0
        end if
...
!-----------------------------------------------------------------------
  ...
  if (istat /= 0) then
     ...
     write(LIS_logunit,*) &
          "*******************************************************"
     write(LIS_logunit,*)"* BAD ", trim(iofunc), " ON FILE ", &
          trim(filename_min6)
...
!----------------------------------------------------------------------------
  ...
  if (twelveyes) then
     ...
     do i = 1, count12
        ! EMK...Support new and old presav files
        if (agrmet_struc(n)%read_expanded_precip_station_ids .eq. 1) then
           read(9, 6001, iostat=istat, err=200, end=200) &
                obs_12(i)%net, obs_12(i)%platform, &
           ...
           obs_12(i)%duration = 99 ! Not saved in old presav files
           obs_12(i)%country_id = "??"
        end if

        if (obs_12(i)%net .eq. "WMO") then
           ! EMK...Support expanded station IDs
           obs_12(i)%wmonum = &
                set_bsn_wmo(agrmet_struc(n)%read_expanded_precip_station_ids, &
                obs_12(i)%platform, obs_12(i)%country_id)
        else if (obs_12(i)%net .eq. "CDMS") then
           read(obs_12(i)%platform, FMT='(I32)') obs_12(i)%wmonum
        else
           obs_12(i)%wmonum = 0
        end if
...
!-----------------------------------------------------------------------
  ...
  if (twelveyes) then
     ...
     write(LIS_logunit,*) &
          '- MAKING 12 HRLY AMTS FROM CURRENT 24 AND OLD 12 HRLY AM  TS'
...
!     ------------------------------------------------------------------
        ...
        if (obs_cur(i)%amt12 <= -99999998) then
           ...
           if (obs_cur(i)%amt24 > -99999998) then
              ...
              call AGRMET_pcpobs_search(obs_cur(i)%net, &
                   obs_cur(i)%platform, &
...
!-----------------------------------------------------------------------
  ...
  if (LIS_masterproc) then
     ...
     do i = 1, fltrcnt
        if (agrmet_struc(n)%read_expanded_precip_station_ids .eq. 1) then
           write(11, 6001, iostat=istat, err=300) &
                obs_cur(i)%net, obs_cur(i)%platform, &
                obs_cur(i)%lat, obs_cur(i)%lon, &
                obs_cur(i)%amt24, obs_cur(i)%amt12, obs_cur(i)%amt6,&
                obs_cur(i)%amtmsc, obs_cur(i)%duration, &
                obs_cur(i)%country_id
        else
     ...
     write(LIS_logunit,*)' '
     write(LIS_logunit,*)'- NUMBER OF 12 AND 6 HOURLY OBS IS ', &
          count12obs, count6obs
     write(LIS_logunit,*)' '
...
!-----------------------------------------------------------------------
  ...
  USE_6 : if ( .not. use_twelve ) then
     ...
     write(LIS_logunit,*) &
          "- PUTTING 6 HOURLY RAIN GAUGE OBSERVATIONS ON GRID."
...
!-----------------------------------------------------------------------
  ...
  USE_12 : if (use_twelve) then
     ...
     write(LIS_logunit,*) &
          "- PUTTING 12 HOURLY RAIN GAUGE OBSERVATIONS ON GRID."
...
! EMK 20 Aug 2021
6001 format(1x, a9, 1x, a31, 1x, f7.2, 1x, f7.2, 1x, ' 24hr ', 1x, &
          i9, 1x, '12hr ', i9, ' 6hr ', i9, ' misc ', i9 &
          ,1x, ' duration ', i2, 1x, ' country ', a2 )
