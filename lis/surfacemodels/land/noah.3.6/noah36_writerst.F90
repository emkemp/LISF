...
!BOP
! 
! !ROUTINE: noah36_writerst
! \label{noah36_writerst}
!
! !REVISION HISTORY:
...
!
! !DESCRIPTION:
!  This program writes restart files for Noah-3.6.
...
! \item[noah36\_dump\_restart](\ref{noah36_dump_restart}) \newline
!   writes the Noah-3.6 variables into the restart file
! \end{description}
!EOP
!  real :: curr_time
  character*100 :: filen
  ...
  write(fnest,'(i3.3)') n
  alarmCheck = LIS_isAlarmRinging(LIS_rc,"Noah36 restart alarm "//trim(fnest))
  ...
  if(alarmCheck .or.(LIS_rc%endtime ==1)) then 
     if ( LIS_masterproc ) then
        ...
        call LIS_create_restart_filename(n,filen,'SURFACEMODEL','NOAH36',&
             wformat=wformat)
        ...
        elseif(wformat.eq."netcdf") then 
           ...
           status = nf90_create(path=filen,cmode=nf90_hdf5,&
               ncid = ftn)
           call LIS_verify(status,'Error in nf90_open in noah36_writerst')
#endif
     ...
     call noah36_dump_restart(n,ftn,wformat)
     ...
     if ( LIS_masterproc ) then
        ...
        elseif(wformat.eq."netcdf") then 
           ...
           call LIS_verify(status,'Error in nf90_close in noah36_writerst')
#endif
        endif
        write(LIS_logunit,*) '[INFO] Noah-3.6 archive restart written: ',filen
     endif
  end if
end subroutine noah36_writerst

!BOP
! 
! !ROUTINE: noah36_dump_restart
! \label{noah36_dump_restart}
!
! !REVISION HISTORY:
!  24 Aug 2004: James Geiger, Initial Specification
! !INTERFACE:
subroutine noah36_dump_restart(n, ftn, wformat)
...
!  \end{description}
!
!  The following is the list of variables written in the Noah-3.6
!  restart file: 
!  \begin{verbatim}
...
!   z0           - Noah-3.6 roughness length
!   emiss        - Noah-3.6 emissivity
!   albedo       - Noah-3.6 albedo
!   snotime1     - Noah-3.6 snow timing variable
!   q1           - Noah-3.6 mixing ratio
!  \end{verbatim}
!
!  The routines invoked are: 
! \begin{description}
...
!EOP
   ...
   call LIS_writeGlobalHeader_restart(ftn,n,LIS_rc%lsm_index,&
        "Noah version 3.6", dimID, dim1=noah36_struc(n)%nslay, &
        dim2= noah36_struc(n)%nslay,&
   ...
   if ( wformat == "netcdf" ) then
      ...
      call LIS_writeHeader_restart(ftn,n,dimID,stcid, "STC",&
           "soil temperature",&
           "K",vlevels=noah36_struc(n)%nslay,valid_min = 220.0, valid_max=330.0,&
           var_flag = "dim2")
      call LIS_writeHeader_restart(ftn,n,dimID,smcid, "SMC",&
           "total soil moisture",&
           "m^3 m-3",vlevels=noah36_struc(n)%nslay,valid_min =0.0, valid_max=0.5,&
           var_flag = "dim1")
      ...
      call LIS_closeHeader_restart(ftn,n,LIS_rc%lsm_index,dimID,noah36_struc(n)%rstInterval)
   endif

   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%t1,&
        varid=t1Id,dim=1,wformat=wformat)

   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%cmc,&
        varid=cmcId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%snowh,&
        varid=snowhId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%sneqv,&
        varid=sneqvId,dim=1,wformat=wformat)
   do l=1,noah36_struc(n)%nslay
      tmptilen = 0.0
      do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
         tmptilen(t) = noah36_struc(n)%noah(t)%stc(l)         
      enddo
   ...
   do l=1,noah36_struc(n)%nslay
      tmptilen = 0.0
      do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
         tmptilen(t) = noah36_struc(n)%noah(t)%smc(l)         
      enddo
      call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,tmptilen,&
   ...
   do l=1,noah36_struc(n)%nslay
      tmptilen = 0.0
      do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
         tmptilen(t) = noah36_struc(n)%noah(t)%sh2o(l)         
      enddo
      call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,tmptilen,&
           varid=sh2oId,dim=l,wformat=wformat)
   enddo

   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%ch,&
        varid=chId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%cm,&
        varid=cmId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%z0,&
        varid=z0Id,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%emiss,&
        varid=emissId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%albedo,&
        varid=albId,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%snotime1,&
        varid=snotime1Id,dim=1,wformat=wformat)
   call LIS_writevar_restart(ftn,n,LIS_rc%lsm_index,noah36_struc(n)%noah%q1,&
...
end subroutine noah36_dump_restart
